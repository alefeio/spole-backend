...
import { listMyArenas } from "../arenas/service";
import { listMyArenasQuerySchema } from "../arenas/schemas";
...
export function usersRoutes(deps: AppDeps) {
  ...
  router.get(
    "/users/me/arenas",
    requireAuth(deps),
    requireRoles(["arena_owner"]),
    async (req, res, next) => {
      try {
        const parsed = listMyArenasQuerySchema.safeParse(req.query);
        if (!parsed.success) {
          return sendFailure(res, 400, "VALIDATION_ERROR", "Invalid query", formatZodError(parsed.error));
        }
        const { data, meta } = await listMyArenas(deps.pool, req.auth!.id, parsed.data);
